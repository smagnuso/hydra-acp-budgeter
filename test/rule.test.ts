...
import { DEFAULT_RULE, loadRule, type BudgetEvent } from "../src/rule.js";
...
test("loadRule imports a JS module's default export", async () => {
  const dir = mkdtempSync(join(tmpdir(), "budgeter-rule-"));
  try {
    const p = join(dir, "rule.js");
    writeFileSync(
      p,
      'export default function(ev) { return ev.kind === "prompt_request" ? { reject: { message: "stop", stopReason: "refusal" } } : null; }\n',
      "utf8",
    );
    ...
    const result = await fn(
      ev({
        kind: "prompt_request",
        budget: { total: 1, perSession: 1, currency: "USD", soft: 5, hard: 10, state: "ok" },
      }),
    );
    assert.deepEqual(result, { reject: { message: "stop", stopReason: "refusal" } });
  } finally {
